...
pub fn day21(input_path: &str) {
    let input = std::fs::read_to_string(input_path).expect("Can't find input file");
    let time = Instant::now();
    //Part 1
    println!("Root yells {}", do_day21_part1(&input));
    //Part 2
    println!("Humn has to yell {}", do_day21_part2(&input));
    println!("{:?}", time.elapsed());
}

fn do_day21_part1(input: &str) -> i64 {
...
fn do_day21_part2(input: &str) -> i64 {
    let mut ids = HashMap::new();
...
hmdt: 32";
    ...
    use super::do_day21_part1;
    use super::do_day21_part2;

    #[test]
    fn part_1() {
        assert_eq!(do_day21_part1(INPUT), 152);
    }
    #[test]
    fn part_2() {
        assert_eq!(do_day21_part2(INPUT), 301);
    }
}
